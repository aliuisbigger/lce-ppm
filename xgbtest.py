...
def getv(data):
    ...
    if len(l)==1:
        ...
        print("ok")
# for filename in range(5,22,1):
#     name = '{}'.format(str(filename))
#     cat_cols = ["Activity", 'org:group']
...
#     t2 = time.time()
#     print(t2 - t1, '秒')
#     X_train, X_test, y_train, y_test = train_test_split(fdata, label, test_size=0.2)
#     t1 = time.time()
#     rf = RandomForestClassifier(n_estimators=30, max_depth=4)
#     rf.fit(X_train, y_train)
#
#     print(cross_val_score(rf, X_test, y_test, cv=3, scoring='f1').mean())
#     t2 = time.time()
#     print(t2 - t1, '秒')
#     X_train, X_test, y_train, y_test = train_test_split(rdata0, label, test_size=0.2)
#     t1 = time.time()
#     rf = RandomForestClassifier(n_estimators=30, max_depth=4)
#     rf.fit(X_train, y_train)
#     print(cross_val_score(rf, X_test, y_test, cv=3, scoring='f1').mean())
...
# print(cross_val_score(rf, X_test, y_test, cv=3, scoring='accuracy').mean())
t2=time.time()
print(t2-t1,'秒')
t1=time.time()
param = {
    'max_depth':range(4, 11, 1),'min_child_weight':range(1,6,2)
}

gs = GridSearchCV(rf,param,cv=3, scoring='f1')
...
#10.csv     {'max_depth': 10, 'min_child_weight': 1}，最好分数0.9290167462410314
#20.csv     最好参数：{'max_depth': 9, 'min_child_weight': 1}，最好分数0.9186615992425499
#30.csv  {'max_depth': 5, 'min_child_weight': 1}，最好分数0.9407893702247357